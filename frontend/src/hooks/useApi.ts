...
const getErrorMessage = (error: unknown): ApiError => {
  ...
  if (error instanceof Error)
    return { message: error.message, isRateLimit: false, retryAfter: 0 };
  if (typeof error === 'string')
    return { message: error, isRateLimit: false, retryAfter: 0 };
...
export function usePlanApi() {
  ...
  const runAction = useCallback(
    async <T>(action: ActionName, request: () => Promise<T>): Promise<T> => {
  ...
  const createSession = useCallback(
    (
      ...
      runAction('createSession', () =>
        planApi.createSession(
          userIntent,
          scenarioName,
          plannerModel,
          executorModel
  ...
  const getSession = useCallback(
    (sessionId: string) =>
      runAction('getSession', () => planApi.getSession(sessionId)),
    [runAction]
  );

  const answerQuestions = useCallback(
    (sessionId: string, answers: Record<string, string>) =>
      runAction('answerQuestions', () =>
        planApi.answerQuestions(sessionId, answers)
      ),
    [runAction]
  );
  ...
  const getProposals = useCallback(
    (sessionId: string) =>
      runAction('getProposals', () => planApi.getProposals(sessionId)),
    [runAction]
  );

  const compareProposals = useCallback(
    (sessionId: string, proposalIds: string[]) =>
      runAction('compareProposals', () =>
        planApi.compareProposals(sessionId, proposalIds)
      ),
    [runAction]
  );
  ...
  const listCandidates = useCallback(
    (sessionId: string) =>
      runAction('listCandidates', () => planApi.listCandidates(sessionId)),
    [runAction]
  );
  ...
  const refineCandidate = useCallback(
    (
      ...
      runAction('refineCandidate', () =>
        planApi.refineCandidate(sessionId, candidateId, payload)
      ),
    [runAction]
  );

  const branchCandidate = useCallback(
    (
      sessionId: string,
      candidateId: string,
      payload?: { title?: string; note?: string }
    ) =>
      runAction('branchCandidate', () =>
        planApi.branchCandidate(sessionId, candidateId, payload)
      ),
    [runAction]
  );
  ...
  const approveCandidate = useCallback(
    (sessionId: string, candidateId: string) =>
      runAction('approveCandidate', () =>
        planApi.approveCandidate(sessionId, candidateId)
      ),
    [runAction]
  );
  ...
  const getOutcomes = useCallback(
    (sessionId: string) =>
      runAction('getOutcomes', () => planApi.getOutcomes(sessionId)),
    [runAction]
  );

  const selectProposal = useCallback(
    (sessionId: string, proposalId: string) =>
      runAction('selectProposal', () =>
        planApi.selectProposal(sessionId, proposalId)
      ),
    [runAction]
  );
  ...
  const approvePlan = useCallback(
    (sessionId: string) =>
      runAction('approvePlan', () => planApi.approvePlan(sessionId)),
    [runAction]
  );
  ...
  const listSessions = useCallback(
    (query?: SessionHistoryQuery) =>
      runAction('listSessions', () => planApi.listSessions(query)),
    [runAction]
  );
