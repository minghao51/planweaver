    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {proposals.map((p) => (
          <div
            key={p.proposal_id}
            className="p-6 rounded-3xl bg-surface-alt border border-white/5 shadow-xl glassmorphism group hover:border-primary/50 transition-all duration-500 relative overflow-hidden"
          >
            ...
            <div className="grid grid-cols-3 gap-2 mb-8">
              <div className="flex flex-col items-center p-3 rounded-2xl bg-black/20 border border-white/5">
                ...
                <span className="text-xs font-bold text-white">
                  {comparison.time_comparison[p.proposal_id]}m
                </span>
                <span className="text-[8px] text-text-muted uppercase font-bold mt-1">
                  Time
                </span>
              </div>
              <div className="flex flex-col items-center p-3 rounded-2xl bg-black/20 border border-white/5">
                ...
                <span className="text-xs font-bold text-white">
                  ${comparison.cost_comparison[p.proposal_id].toFixed(2)}
                </span>
                <span className="text-[8px] text-text-muted uppercase font-bold mt-1">
                  Cost
                </span>
              ...
              <div className="flex flex-col items-center p-3 rounded-2xl bg-black/20 border border-white/5">
                ...
                <span className="text-xs font-bold text-white uppercase">
                  {comparison.complexity_comparison[p.proposal_id]}
                </span>
                <span className="text-[8px] text-text-muted uppercase font-bold mt-1">
                  Risk
                </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="p-8 rounded-3xl bg-surface border border-white/5 glassmorphism h-fit">
            ...
            <div className="space-y-3">
              {comparison.common_steps.map((step, i) => (
                <div
                  key={i}
                  className="flex items-center gap-3 p-3 rounded-xl bg-white/5 border border-white/5 text-sm font-medium text-text-body"
                >
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            {Object.entries(comparison.unique_steps_by_proposal).map(
              ([propId, steps]) => (
                <div
                  key={propId}
                  className="p-6 rounded-3xl bg-surface-alt border border-white/5 glassmorphism"
                >
                  ...
                  {steps.length > 0 ? (
                    <div className="space-y-2">
                      {steps.map((step, i) => (
                        <div
                          key={i}
                          className="flex items-center gap-3 p-2.5 rounded-lg bg-black/20 text-xs font-medium text-text-body italic"
                        >
                    ...
                    <p className="text-xs text-text-muted italic opacity-40 py-2">
                      No divergent maneuvers identified.
                    </p>
                  )}
                </div>
              )
