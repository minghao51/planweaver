    ...
    try:
        ...
        for migration in MIGRATIONS:
            if migration["version"] > current_version:
                logger.info(
                    f"Running migration v{migration['version']}: {migration['name']}"
                )
                try:
    ...
    try:
        ...
        if session_ids:
            db.query(ExecutionLog).filter(
                ExecutionLog.session_id.in_(session_ids)
            ).delete(synchronize_session=False)
            db.query(OptimizedVariant).filter(
                OptimizedVariant.session_id.in_(session_ids)
            ).delete(synchronize_session=False)
            db.query(NormalizedPlanRecord).filter(
                NormalizedPlanRecord.session_id.in_(session_ids)
            ).delete(synchronize_session=False)
            db.query(PlanEvaluationRecord).filter(
                PlanEvaluationRecord.session_id.in_(session_ids)
            ).delete(synchronize_session=False)
            db.query(PairwiseComparisonRecord).filter(
                PairwiseComparisonRecord.session_id.in_(session_ids)
            ).delete(synchronize_session=False)
            db.query(PlanRating).filter(PlanRating.session_id.in_(session_ids)).delete(
                synchronize_session=False
            )
            db.query(UserRating).filter(UserRating.session_id.in_(session_ids)).delete(
                synchronize_session=False
            )
            db.query(PlanModel).filter(PlanModel.session_id.in_(session_ids)).delete(
                synchronize_session=False
            )
            db.query(SessionModel).filter(SessionModel.id.in_(session_ids)).delete(
                synchronize_session=False
            )
            db.commit()
            logger.info(
                f"Cleaned up {deleted_count} expired sessions (older than {ttl_days} days)"
            )
